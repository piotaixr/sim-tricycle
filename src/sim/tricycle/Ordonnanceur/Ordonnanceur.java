...
public class Ordonnanceur implements OrdonnanceurInterface {
    ...
    public Ordonnanceur(long time) {
        ...
        subscribersActionDone = new ArrayList();
        timer = new Timer();
        timer.schedule(new TaskAction(), 0, time);
    ...
    /*
     * Ajoute en fin de la liste des objets ayant des actions
     * Utilisée pour transférer un élément de la liste à faire à celle de ceux faits
     */
   ...
   /*
    * Enlève en tête de la liste des objets ayant des actions et le met dans la liste des actions effectuées
    */
    private void actionAndRemoveFromToDo(){
        OrdonnancableInterface Elem = subscribersActionToDo.remove(0);
        doAction(Elem);
        addToActionDone(Elem);
    }

    /*
     * Remplissage des actions à faire de manière aléatoire
     * A faire une fois qu'il n'y a plus d'action à faire
     */
    private void intializeActionToDo(){
        ...
        do{
            ...
            /* On enlève un élément de la liste des actions faites, dont l'index est le modulo du nombre aléatoire par 
             le nombre d'éléments restants dans la liste que l'on met dans la liste des actions à faire*/
        ...
        public void run() {
            if (running) {
                if (!subscribersActionToDo.isEmpty())
                    ...
                    /* Plus d'action à faire donc on transvase les actions dans le tableau des actions à faire de manière aléatoire */
                    intializeActionToDo();
            }
        }
    }
    ...
    public void remove(OrdonnancableInterface oi) {
        /* Si l'element à enlever est dans la liste des éléments restants à traiter on l'enlève (ex: un bot tue une autre bot, il ne faut 
         pas que celui ci puisse faire une action une fois mort)*/
        if (subscribersActionToDo.contains(oi))
            subscribersActionToDo.remove(subscribersActionToDo.indexOf(oi));
        /* Sinon on l'enlève des éléments déjà traités */
        else
            subscribersActionDone.remove(subscribersActionDone.indexOf(oi));
    }

    /*
     * Ajoute l'objet en paramètre à la fin de la liste des actions à faire
     * Ex : Création nouveau bot
     */
    @Override
